...
CALL SelectCountry ('Afghanistan', '');

 
-- Which countries have higher than 50% in all three wash variables? 
-- For viewing a range, add starting and ending year otherwise write 0. 

DROP PROCEDURE IF EXISTS FiftyPercentWash;

DELIMITER //
CREATE PROCEDURE FiftyPercentWash(
	IN x int,
	IN y int) 
...
Call FiftyPercentWash(2015, 2016);
...
SELECT Period, Area_Divide, ROUND(AVG(Percent_OpenDefecation)), Count(Country)
	FROM wash_table
